...
public static class DataManager {
    ...
    public static int GetPlayerKey() {
        return PlayerPrefs.GetInt("PLAYER_KEY", 0);
    }

    public static void SetPlayerKey(int num) {
        PlayerPrefs.SetInt("PLAYER_KEY", num);
    }
    ...
    public static int GetSelectedSkin() {
        return PlayerPrefs.GetInt("PLAYER_SELECTED_SKIN", 0);
    }
    ...
    public static int GetStageRotation() {
        return PlayerPrefs.GetInt("STAGE_ROTATION", 0);
    }

    public static void SetStageRotation(int count) {
        PlayerPrefs.SetInt("STAGE_ROTATION", count);
    }

    public static int GetStagePhase() {
        return PlayerPrefs.GetInt("STAGE_PHASE", 1);
    }

    public static void SetStagePhase(int count) {
        PlayerPrefs.SetInt("STAGE_PHASE", count);
    }
    ...
    /// </summary>
    /// <returns></returns>
    public static int GetUserStageClearLevel() {
        return PlayerPrefs.GetInt("USER_STAGECLEAR_LEVEL", 1);
    }

    public static void SetUserStageClearLevel(int count) {
        PlayerPrefs.SetInt("USER_STAGECLEAR_LEVEL", count);
    }
    ...
    /// </summary>
    /// <returns></returns>
    public static int GetUserRandomSkinBuy() {
        return PlayerPrefs.GetInt("USER_RANDOM_SKIN_BUY", 0);
    }

    public static void SetUserRandomSkinBuy(int count) {
        PlayerPrefs.SetInt("USER_RANDOM_SKIN_BUY", count);
    }

    // ステージ作成経験があるか？
    public static void SetStageCreateFlag(bool newVal) {
        SetBool("STAGE_CREATED", newVal);
    }

    // ステージ作成経験があるか？チェック
    public static bool GetStageCreateFlag() {
        return GetBool("STAGE_CREATED", false);
    }

    // 鍵を拾ったか？
    public static void SetUserGotKey(bool newVal) {
        SetBool("PLAYER_GOT_KEY", newVal);
    }

    // 鍵を拾ったか？
    public static bool GetUserGotKey() {
        return GetBool("PLAYER_GOT_KEY", false);
    }


    // 初プレイか？
    public static void SetFirstPlay(bool newVal) {
        SetBool("MY_FIRST_PLAY", newVal);
    }

    // 初プレイか？
    public static bool GetFirstPlay() {
        return GetBool("MY_FIRST_PLAY", true);
    }


    // ステージデータのチェック
    public static int GetUserStageData(string stageType, int index) {
        return PlayerPrefs.GetInt("STAGEDATA_" + stageType + index, -1);
    }

    // ステージデータの書き込み
    public static void SetUserStageData(string stageType, int index, int val) {
        PlayerPrefs.SetInt("STAGEDATA_" + stageType + index, val);
    }

    // ステージ使用歴のチェック
    public static List<int> GetUserPlayedStageData(string stageType) {

        // 保存されている使用歴を取得（カンマ区切りの文字列）
        string saveData = PlayerPrefs.GetString("STAGE_USED_" + stageType, "");
        List<int> returnData = new List<int>();

        if(saveData==null || saveData == "") { return null; }

        // 文字列をカンマでスプリット
        var splitData = saveData.Split(','); 

        foreach(string obj in splitData) {
            int stageID = int.Parse(obj);//キャスト
            returnData.Add(stageID);
        }
    ...
    // ステージ使用歴の書き込み
    public static void SetUserPlayedStageData(string stageType, List<int> newData) {

        string saveData = "";

        // まず与えられたリストをカンマで区切りながら文字列を作成する
        for(int i=0; i<newData.Count; i++) {
            if (i == 0) { saveData += newData[i].ToString(); }
            else { saveData += "," + newData[i]; }
        }

        PlayerPrefs.SetString("STAGE_USED_" + stageType, saveData);

    }

    public static void ClearUserPlayedStageData(string stageType) {
        PlayerPrefs.SetString("STAGE_USED_" + stageType, "");
    }

    // ステージ使用歴のチェック
    public static List<int> GetUserPlayedBossStageData(string stageType) {

        // 保存されている使用歴を取得（カンマ区切りの文字列）
        string saveData = PlayerPrefs.GetString("BOSS_STAGE_USED_" + stageType, "");
        List<int> returnData = new List<int>();
    ...
    // ステージ使用歴の書き込み
    public static void SetUserPlayedBossStageData(string stageType, List<int> newData) {

        string saveData = "";

        // まず与えられたリストをカンマで区切りながら文字列を作成する
        for (int i = 0; i < newData.Count; i++) {
            if (i == 0) { saveData += newData[i].ToString(); } else { saveData += "," + newData[i]; }
        }

        PlayerPrefs.SetString("BOSS_STAGE_USED_" + stageType, saveData);

    }
    public static void ClearUserPlayedBossStageData(string stageType) {
        PlayerPrefs.SetString("BOSS_STAGE_USED_" + stageType, "");
    }

    public static int GetRateIOS() {
        return PlayerPrefs.GetInt("RATEUS_COUNT", 0);
    }

    public static void SetRateIOS(int count) {
        PlayerPrefs.SetInt("RATEUS_COUNT", count);
    }

	public static int GetRateWindowCount () {
		return PlayerPrefs.GetInt("RATEUS_WINDOW_COUNT", 0);
	}

	public static void SetRateWindowCount (int count) {
		PlayerPrefs.SetInt("RATEUS_WINDOW_COUNT", count);
	}
